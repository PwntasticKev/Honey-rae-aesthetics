...
export const executeWorkflowStep = internalMutation({
  ...
  handler: async (ctx, args) => {
    console.log(
      `🚀 Executing workflow step: ${args.stepType} for enrollment ${args.enrollmentId}`,
    );
...
function substituteVariables(
  ...
  result = result.replace(
    /\{\{first_name\}\}/g,
    client.firstName || client.fullName?.split(" ")[0] || "there",
  );
  result = result.replace(
    /\{\{last_name\}\}/g,
...
async function executeConditionalStep(ctx: any, data: any) {
  ...
  switch (field) {
    ...
    case "appointment_count":
      ...
      conditionMet = evaluateNumericCondition(
        appointmentCount,
        operator,
        parseInt(value),
      );
    ...
    case "appointment_type":
      ...
      break;
    case "client_status":
      conditionMet = evaluateStringCondition(
    ...
    case "last_appointment_date":
      ...
      conditionMet = evaluateDateCondition(
        lastAppointmentDate,
        operator,
        value,
      );
...
async function getClientAppointmentCount(
  ctx: any,
  clientId: string,
): Promise<number> {
