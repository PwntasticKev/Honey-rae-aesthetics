...
export const setupDemo = mutation({
	...
	handler: async (ctx) => {
		...
		await ctx.db.insert("messageTemplates", {
			...
			type: "sms",
			content: "Hi {{first_name}}, this is a reminder for your appointment tomorrow at {{appointment_time}}. Please arrive 10 minutes early. Call us if you need to reschedule.",
		...
		await ctx.db.insert("workflows", {
			...
			description: "Automatically welcome new clients with a series of messages",
			trigger: "new_client",
			conditions: [],
			actions: [
				{
