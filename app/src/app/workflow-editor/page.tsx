...
export default function WorkflowEditorPage() {
	...
	const realWorkflow = isValidConvexId ? useQuery(
		api.workflows.get, 
		{ id: workflowId as any }
	) : undefined;

	// Mock workflow data - only use for non-Convex IDs
	const mockWorkflow = workflowId && !workflowId.startsWith('workflows:') ? {
		id: workflowId,
	...
	const handleSave = async (workflow: any) => {
		try {
			...
			console.log("Is valid Convex ID:", workflowId && workflowId.startsWith('workflows:'));
	...
	const transformedRealWorkflow = realWorkflow ? {
		...
		blocks: realWorkflow.actions?.map((action: any, index: number) => ({
			id: `action_${index}`,
			type: action.type,
	...
	console.log('isValidConvexId:', workflowId && workflowId.startsWith('workflows:'));
