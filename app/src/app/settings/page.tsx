...
export default function SettingsPage() {
  ...
  const apiIntegrations = Object.entries(platformDefinitions).map(
    ([platformId, platform]) => {
      const integration = userIntegrations?.find(
        (i) => i.platform === platformId,
      );
      return {
        id: platformId,
        name: platform.name,
  ...
  const handleConnectIntegration = async (integrationId: string) => {
    ...
    if (socialPlatforms.includes(integrationId)) {
      try {
        ...
        alert(
          `Failed to connect to ${integrationId}. Please ensure your API credentials are configured.`,
        );
      }
    } else {
      // Handle API key based integrations (Stripe, Twilio, etc.)
      const apiKey = prompt(
        `Enter your ${platformDefinitions[integrationId as keyof typeof platformDefinitions]?.name} API key:`,
      );
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {(() => {
                        ...
                        const validation =
                          validateOrganizationProfile(profileData);
                        const progress =
                          getOrganizationSetupProgress(profileData);
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm">
                                    ...
                                    {orgSettings.name || "Unnamed Organization"}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                                ...
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`w-2 h-2 ${user?.orgId ? "bg-green-500" : "bg-red-500"} rounded-full`}
                                  ></div>
                                  <span className="text-sm">
                                    Organization Linked:{" "}
                                    {user?.orgId ? "Yes" : "No"}
                                ...
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`w-2 h-2 ${validation.isValid ? "bg-green-500" : "bg-yellow-500"} rounded-full`}
                                  ></div>
                                  <span className="text-sm">
                                    Profile Complete:{" "}
