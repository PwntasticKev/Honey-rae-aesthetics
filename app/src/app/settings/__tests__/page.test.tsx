...
vi.mock("@/components/Sidebar", () => ({
  Sidebar: ({ isOpen }: { isOpen: boolean }) => (
    <div
      data-testid="sidebar"
      className={`fixed left-0 top-0 w-64 h-full ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
    >
      Sidebar
    </div>
  ),
...
describe("Settings Page Layout", () => {
  ...
  it("should show organization setup validation errors when missing required fields", async () => {
    ...
    const orgSetupTab = screen.getByRole("tab", {
      name: /organization setup/i,
    });
