...
export function ClientTimeline({ orgId, clientId }: ClientTimelineProps) {
  ...
  // Note: appointments and messages don't have getByOrg queries yet
  // const appointments = useQuery(api.appointments.getByOrg, orgId ? { orgId: orgId as any } : "skip") || [];
  // const messages = useQuery(api.messages.getByOrg, orgId ? { orgId: orgId as any } : "skip") || [];
  const notes =
    useQuery(api.notes.getByOrg, orgId ? { orgId: orgId as any } : "skip") ||
    [];
          ...
          {filteredEvents.length === 0 ? (
            ...
            filteredEvents.map((event, index) => (
              ...
              >
                {/* Timeline Dot */}
                <div
                  className={`relative z-10 flex-shrink-0 w-16 h-16 rounded-full border-2 flex items-center justify-center ${getEventColor(event.type)}`}
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        {event.status && (
                          <span
                            className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(event.status)}`}
                          >
                            {event.status}
                          </span>
                        )}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4">
            <div className="flex justify-between items-center p-6 border-b">
              <div className="flex items-center space-x-3">
                <div
                  className={`p-2 rounded-lg ${getEventColor(selectedEvent.type)}`}
                >
              ...
              <div className="space-y-4">
                ...
                {selectedEvent.status && (
                  <div>
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${getStatusColor(selectedEvent.status)}`}
                    >
                      {selectedEvent.status}
                    </span>
                  </div>
